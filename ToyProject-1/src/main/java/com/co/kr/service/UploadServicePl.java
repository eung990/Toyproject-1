...
@Service
@Transactional
public class UploadServicePl implements UploadService {
	...
	public int fileProcess(FileListVO fileListVO, MultipartHttpServletRequest request, HttpServletRequest httpReq) {
		...
		if (fileListVO.getIsEdit() != null) {
			...
			// db 인서트
			uploadMapper.bdContentUpload(boardContentDomain);
		...
		System.out.println("현재 session에 있는 값" + session.getAttribute("files"));
		...
		for (MultipartFile multipartFile : multipartFiles) {
			...
			if (!multipartFile.isEmpty()) { // 파일 있을때
				...
				try {
					...
					// db 인서트
					uploadMapper.bdFileUpload(boardFileDomain);
	...
	public void bdContentRemove(HashMap<String, Object> map) {
		uploadMapper.bdContentDelete(map);
	}

	@Override
	public void bdFileRemove(BoardFileDomain boardFileDomain) {
		uploadMapper.bdFileDelete(boardFileDomain);
	}

	@Override
	public BoardListDomain bdSelectOne(HashMap<String, Object> map) {
		return uploadMapper.bdSelectOne(map);
	}

	@Override
	public List<BoardFileDomain> bdSelectOneFile(HashMap<String, Object> map) {
		return uploadMapper.bdSelectOneFile(map);
	}
